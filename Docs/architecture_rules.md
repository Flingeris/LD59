...
Правильный поток действия ночью теперь строится вокруг позиции хранителя и world interaction.
...
Это базовый паттерн новой версии ночной фазы.
---
...
## Keeper / POI Rule
Новая глубина Bellgrave строится вокруг хранителя и точек интереса.

Правила:
- хранитель является частью gameplay, а не только presentation;
- доступность ночных действий зависит от позиции хранителя;
- точки интереса должны быть явными runtime / scene сущностями;
- действия не должны срабатывать мгновенно без учёта перемещения;
- если ограничение можно выразить через keeper state и POI state, оно не должно размазываться по UI.

Важно:
позиция хранителя — это gameplay-истина, а не просто анимация.
...
- этот паттерн используется для bell objects, points of interest и других интерактивных world objects, если нет отдельной причины сделать иначе.